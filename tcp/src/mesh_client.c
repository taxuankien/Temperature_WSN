...
static void ble_mesh_custom_sensor_client_model_cb(esp_ble_mesh_model_cb_event_t event,
    ...
    switch (event) {
        case ESP_BLE_MESH_MODEL_OPERATION_EVT:
            switch (param->model_operation.opcode) {
                case ESP_BLE_MESH_CUSTOM_SENSOR_MODEL_OP_STATUS:
                    ESP_LOGI(TAG, "OP_STATUS -- Mensagem recebida: 0x%06lx", param->model_operation.opcode);
        ...
        case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
            ...
            ESP_LOGI(TAG, "Send message opcode 0x%06lx success!", param->model_send_comp.opcode);
        ...
        case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT:
            switch (param->client_recv_publish_msg.opcode) {
                ...
                default:
                    ESP_LOGW(TAG, "Received unrecognized OPCODE message: 0x%04lx", param->client_recv_publish_msg.opcode);
                break;
            }
        ...
        case ESP_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT:
            ESP_LOGW(TAG, "Message opcode 0x%06lx timeout", param->client_send_timeout.opcode);
...
esp_err_t ble_mesh_custom_sensor_client_model_message_set(model_sensor_data_t set_data, uint16_t addr) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to send the custom msg 0x%06lx, err = 0x%06x", opcode, err);
    }
